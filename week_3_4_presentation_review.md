...
| *How does the drift detection work?* | "We run a statistical Kolmogorov-Smirnov test on numerical features and a Chi-Square test on categorical features. We compare the distribution of the incoming production inference data against our baseline training dataset. If the p-value falls below 0.05, we conclude that the data has drifted." |
